...
public class FriendsCommand extends Command implements TabExecutor {
    ...
    protected final BungeeBridgeFriends plugin;

    public FriendsCommand(FriendsManagement friendsManagement, BungeeBridgeFriends plugin) {
        ...
        BungeeBridge.getInstance().getExecutor().addTask(() -> {
                if (args.length == 0) {
                    ...
                    if (command.equalsIgnoreCase("help")) {
                        ...
                        UUID target;
                        try {
                            target = plugin.getUuidTranslator().getUUID(arg, false);
                        } catch (Exception e) {
                        ...
                        for (Map.Entry<UUID, String> entry : list.entrySet()) {
                            ...
                            boolean isOnline = plugin.getNetworkBridge().isOnline(entry.getKey());
